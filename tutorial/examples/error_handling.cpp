...
int test5(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //this is old error check code, uses assert as a macro
        /*
        GLClearError(); //clear out errors 

        //how is index buffer rendering this different that glDrawArrays, how does it know what to render first 
        glDrawElements(GL_TRIANGLES, 6, GL_INT, nullptr);

        ASSERT(GLLogCall()); //print out the created error of GL_INT instead of using GL_UNSIGNED_INT
        //write a function to translate the decimal code, to hex, which relates to an error message 
        //as assert here places a breakpoint that will stop execution 
        */ 
